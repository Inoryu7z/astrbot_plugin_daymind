...
请直接输出 JSON，不要有其他内容。"""
    ...
    def get_mood_style_text(self, mood: dict) -> str:
        """
        获取心情风格注入文本
        ...
        if strength == "弱":
            # 弱强度：只轻微影响
            style_text = f"你当前心情偏{label}。回复时可以轻微体现，但不要过于明显。{reason}"
        elif strength == "强":
            # 强强度：更明显的风格
            style_text = f"你当前心情是{label}。{base_style} {reason}"
        else:
            # 中等强度（默认）
            style_text = f"你当前心情偏{label}。{base_style} {reason}"
    ...
    def build_mood_injection(self, mood: dict) -> str:
        """
        ...
        style_text = self.get_mood_style_text(mood)
        if not style_text:
            return ""
